...
- Select `Local file`, click the button to upload files, and navigate to the downloaded OVA file on your local machine ( `photon-3-kube-v1.24.9+vmware.1-tkg.1-f5e94dab9dfbb9988aeb94f1ffdc6e5e.ova` and/or `ubuntu-2004-kube-v1.24.9+vmware.1-tkg.1-b030088fe71fea7ff1ecb87a4d425c93.ova` ).
...
When the OVA deployment finishes, right-click the VM ( `photon-3-kube-v1.24.9+vmware.1` and/or `ubuntu-2004-efi-kube-v1.24.9+vmware.1` ) and select `Template` -> `Convert to Template`.
...
Copy the VMware Tanzu CLI binary to a location which is in the system path, for example: `/usr/local/bin` ( `cp ~/tanzu-cli/cli/core/v0.28.0/tanzu-core-linux_amd64 /usr/local/bin/tanzu` ).
...
### Step 8
Copy the downloaded kubectl cluster cli for Linux ( `kubectl-linux-v1.24.9+vmware.1.gz` ) file to the linux based environment with `docker` installed on it.

---

### Step 9
`gunzip` the kubectl cluster cli ( `kubectl-linux-v1.24.9+vmware.1.gz` )

Move the extracted file to a location which is in the system path, for example: `/usr/local/bin` ( `mv /path/to/kubectl-linux-v1.24.9+vmware.2 /usr/local/bin/kubectl` ).
...
Create the *management* cluster by running the following command ( takes approx 20 mins to complete ):
