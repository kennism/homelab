# Install/Configure Tanzu Application Platform `tap` on `aks`

### This document describes how to install/configure `tap` ( [Tanzu Application Platform](https://docs.vmware.com/en/VMware-Tanzu-Application-Platform) ) version `1.0.1` on an `aks` cluster ( and run a demo workload ).
...
Setup/configure `aks` cluster, roles and resources. Create the `aks` cluster, roles and resources on `azure` by executing the steps below to create an `aks` cluster called `tap-demo`.

*NOTE: Make sure that the available quota for "Standard DSv2 Family vCPUs" is at least 16 ( see: https://portal.azure.com/#blade/Microsoft_Azure_Capacity/QuotaMenuBlade/myQuotas )*

```
az login
az group create --location northeurope --name tap-demo
az extension add --name aks-preview
az feature register --name PodSecurityPolicyPreview --namespace Microsoft.ContainerService
az feature list -o table --query "[?contains(name, 'Microsoft.ContainerService/PodSecurityPolicyPreview')].{Name:name,State:properties.state}"
az provider register --namespace Microsoft.ContainerService
az aks create --resource-group tap-demo --name tap-demo --node-count 4 --node-vm-size Standard_DS3_v2 --node-osdisk-size 500 --enable-pod-security-policy --generate-ssh-keys
az aks get-credentials --resource-group tap-demo --name tap-demo
```

*NOTE: when running the `azure-cli` in a docker container ( https://docs.microsoft.com/en-us/cli/azure/run-azure-cli-docker ) make sure to copy `/root/.kube/config` to your local machine where you have `kubectl` installed using, for example, a command similar to this: `docker cp containerid:/root/.kube/config aks-config`*

After completion of the script, check if the context was added to the kube config:

`kubectl config get-contexts`

![](images/azure-kubectl-contexts.png)

List which pods are running on the clusters:

`kubectl get pods -A`

![](images/azure-kubectl-pods.png)
...
Create an `acr` container registry on `azure`.

In the `acr` section of the `azure` console ( https://portal.azure.com/#blade/HubsExtension/BrowseResourceBlade/resourceType/Microsoft.ContainerRegistry%2Fregistries ). Click on `Create` and go through the wizard.
...
`kubectl create secret docker-registry registry-credentials --docker-server='https://fancyregistryname.azurecr.io' --docker-username='00000000-0000-0000-0000-000000000000' --docker-password='topSecretPassword' -n dev`
